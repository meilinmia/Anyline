...
class TEDDetector:
    ...
    @classmethod
    def from_pretrained(cls, local_ckpt_path=None):
        
        #local_ckpt_path: 绝对路径或相对当前文件的路径
        if local_ckpt_path is None:
            # 默认路径：指向你的 ckpts 目录
            local_ckpt_path = os.path.join(
                os.path.dirname(__file__),   # teed 文件夹
                "..", "..", "..",        # 再回到 comfyui_controlnet_aux-main
                "ckpts",
                "TheMistoAI",
                "MistoLine",
                "Anyline" ,
            ...
            local_ckpt_path = os.path.abspath(local_ckpt_path)

        if not os.path.isfile(local_ckpt_path):
            raise FileNotFoundError(f"本地未找到 MTEED 权重: {local_ckpt_path}")
